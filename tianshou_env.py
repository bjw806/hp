...
def basic_reward_function(history):
    ...
    매 스텝에서:
      1) 현재 스텝의 실현 PnL을 구함
      2) 최근 N스텝의 실현 PnL 표준편차를 구해서 페널티로 적용
      3) 보상 = (현재 실현 PnL) - α * (최근 PnL 표준편차)
    """
    # 1. 현재 스텝의 실현 손익
    current_pnl = history["realized_pnl", -1]

    # 2. 최근 N 스텝의 변동성(표준편차) 계산
    N = 30  # 혹은 10, 50 등 원하는 길이
    realized_pnl_arr = history["realized_pnl"][-N:]  # 최근 N개 스텝 PnL
    if len(realized_pnl_arr) <= 1:
        rolling_std = 0.0
    else:
        rolling_std = np.std(realized_pnl_arr)

    # 3. 보상 계산 (alpha는 위험에 대한 페널티 강도)
    alpha = 0.1  # 필요에 맞춰 조절
    reward = current_pnl - alpha * rolling_std
...
class DiscretedTradingEnv(gym.Env):
    ...
    def step(self, position_index=None):
        ...
        if self.liquidation:
            ...
            reward = (
                self.reward_function(self.historical_info) + 0.1 + no_position_panelty
            )
